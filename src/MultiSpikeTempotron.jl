...
"""
    get_critical_threshold(m::Tempotron, PSPs, PSP, y₀[, tol = 1e-13])
...
"""
function get_critical_threshold(
    m::Tempotron,
    PSPs::Array{Tp,1},
    y₀::Integer,
    tol::Real = 1e-13,
    ...
    # by using θ₁.
    @inline function v_max_linked_to_spike(
        spikes1::Array{Tp1,1},
        spikes2::Array{Tp1,1},
        v_max2::Tp2,
    ) where {Tp1<:Any,Tp2<:NamedTuple}
    ...
    while k₁ ≠ y₀ || k₂ ≠ (y₀ - 1) || !v_max_linked_to_spike(spikes1, spikes2, v_max2)
        θ = (θ₁ + θ₂) / 2
        spk, v_max1 = get_spikes(m, PSPs, θ, return_v_max = true)
        if v_max1.v_max ≤ 0
            throw(NonPositiveVoltageError())
        end
        k = length(spk)
        if k < y₀
            θ₂, k₂, spikes2, v_max2 = θ, k, spk, v_max1
        else
            θ₁, k₁, spikes1 = θ, k, spk
        end
    end
    ...
    (v_max_psp, t_max, next_psp, ~, sum_m, sum_s, ΔTϵ) = v_max2
    ...
    @inline function v_max(θ::Real)::Real
        spk = get_spikes(m, PSPs_max, θ, M).spikes
        sum_e = isempty(spk) ? 0.0 : sum(x -> exp.((x.time - ΔTϵ) ./ m.τₘ), spk)
        t_max_θ = get_next_t_max(m, v_max_j, next_psp, ΔTϵ, sum_m, sum_s, sum_e, θ)[1]
...
"""
function get_θ_gradient(
    m::Tempotron{N},
    inp::SpikesInput{T1,N},
    spk::Array{T2,1},
    PSP::Function,
    dPSP::Function,
)::Array{Real,1} where {T1<:Real,N,T2<:Real}
...
function train_∇!(
    ...
    # TODO: Performance?
    @inline PSP(t::Real)::Real = sum(x -> x.ΔV(t), PSPs)
    @inline dPSP(t::Real)::Real = sum(x -> m.w[x.neuron] * m.K̇(t - x.time), PSPs)
...
function get_sts(
    ...
    return get_sts(m, convert(SpikesInput, inp); kwargs...)
end
...
function pretrain!(
    ...
    while μ < μₜ
        block = [
            (
                x = poisson_spikes_input(N, ν = ν_in, T = T),
