...
hps = Plots.Plot[]
y_lims = []
for k = 1:length(teachers)
    hp = plot!(hist_t, R_means[k], ribbon = R_stds[k], color = cols',
               legend = false, xscale = :log10, subplot = 1 + k,
               inset = (1,
                        bbox(0.7(k - 1) / length(teachers), 0.0,
                             1 / length(teachers), 1.0, :bottom, :left)))
    if k > 1
...
y_lims = (minimum(x -> x[1], y_lims), maximum(x -> x[2], y_lims))
for hp ∈ hps
    ylims!(hp, y_lims)
end
